...
'''
    ...
    try:
        ...
        subprocess.run(
            ["pkill", "-f", f"shamir-party.x.*{test_name}"], capture_output=True
        )
...
class TestMatSatClauseWeights:
    ...
    def test_conflict_resolution(self):
        ...
        assert (
            assignment[0] == 1
        ), "x0 should be 1 due to higher weight (100 vs 1) on positive clause"
        assert (
            assignment[1] == 0
        ), "x1 should be 0 due to higher weight (100 vs 1) on negative clause"
        assert (
            assignment[2] == 1
...
'''
    ...
    def test_sum_of_weights(self):
        ...
        assert not (
            assignment[0] == 0 and assignment[1] == 0
        ), "Scenario 2 failed: Should perform better than (0,0)"
        valid_others = (assignment[0] == 0 and assignment[1] == 1) or (
